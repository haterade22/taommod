...
const TEMPLATE_BUILD_ORDERS: Record<string, BuildOrder[]> = {
  OneHandedSword:    [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Guard', order: 1 }, { pieceType: 'Blade', order: 2 }, { pieceType: 'Pommel', order: -1 }],
  TwoHandedSword:   [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Guard', order: 1 }, { pieceType: 'Blade', order: 2 }, { pieceType: 'Pommel', order: -1 }],
  Dagger:            [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Guard', order: 1 }, { pieceType: 'Blade', order: 2 }, { pieceType: 'Pommel', order: -1 }],
  OneHandedAxe:      [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  TwoHandedAxe:      [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  Mace:              [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  TwoHandedMace:     [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  TwoHandedPolearm:  [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  Pike:              [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  Javelin:           [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  ThrowingAxe:       [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
  OneHandedPolearm:  [{ pieceType: 'Handle', order: 0 }, { pieceType: 'Blade', order: 1 }, { pieceType: 'Pommel', order: -1 }],
};
...
function simulateSwingLayer(
  weaponReach: number, wideGrip: boolean,
  angleSpan: number, usablePower: number, maxTorque: number, inertia: number,
): { finalSpeed: number; finalTime: number } {
  let angle = 0, speed = 0.01, time = 0;
...
function simulateThrustLayer(
  distance: number, usablePower: number, maxForce: number, mass: number,
): { finalSpeed: number; finalTime: number } {
  let pos = 0, speed = 0.01, time = 0;
  while (pos < distance) {
...
function calcSwingSpeed(reach: number, weight: number, inertia: number, com: number, flags: WeaponFlags): number {
  ...
  let p1 = 170, p2 = 90, t1 = 27, t2 = 15, t3 = 7;

  if (isTwoHanded) {
    if (isWide)  { I += 1.5; t3 *= 4.0; t2 *= 1.7; p2 *= 1.3; p1 *= 1.15; }
    else         { I += 1.0; t3 *= 2.4; t2 *= 1.3; p2 *= 1.35; p1 *= 1.15; }
  }
...
function calcThrustSpeed(weight: number, inertia: number, com: number, flags: WeaponFlags): number {
  ...
  let p1 = 170, p2 = 90, f1 = 24, f2 = 15;

  if (isTwoHanded && !isWide)  { mass += 0.6; f2 *= 1.9; f1 *= 1.1; p2 *= 1.2; p1 *= 1.05; }
  else if (isTwoHanded && isWide) { mass += 0.9; f2 *= 2.1; f1 *= 1.2; p2 *= 1.2; p1 *= 1.05; }
...
function strikeMagnitudeSwing(
  swingSpeed: number, impactPt: number,
  weight: number, length: number, inertia: number, com: number,
): number {
...
function baseBlowMagnitudeSwing(
  angularSpeed: number, reach: number, weight: number, inertia: number, com: number, impactPt: number,
): number {
...
function assembleWeapon(
  pieces: Map<string, ScaledPiece>, buildOrders: BuildOrder[],
  ...
  let totalInertia = 0, offset = -centerOfMass;
  for (const bo of buildOrders) {
    const p = pieces.get(bo.pieceType);
    if (!p) continue;
...
export function computeCraftedWeaponDamage(
  ...
  for (let ip = 0.93; ip > 0.5; ip -= 0.05) {
    const mag = baseBlowMagnitudeSwing(swingSpeed, stats.weaponReach, stats.totalWeight, stats.inertia, stats.centerOfMass, ip);
...
export function computeRawDamage(damageType: string, magnitude: number, armor: number): number {
  ...
  switch (damageType) {
    case 'Cut':    bluntFactor = 0.1;  armorThreshold = 0.5;  break;
    case 'Pierce': bluntFactor = 0.25; armorThreshold = 0.33; break;
    case 'Blunt':  bluntFactor = 0.6;  armorThreshold = 0.2;  break;
    default:       bluntFactor = 0.1;  armorThreshold = 0.5;
  }
